...
namespace AKIRA.UIFramework {{
    ...
    public partial class {name}Panel : {name}PanelProp {{
...
#region 辅助事件
        ...
        internal static StringBuilder LinkControlContent(Transform _transform) {
            ...
            for (int i = 0; i < nodes.Count - 1; i++) {
                ...
                var propRenameComponent = _transform.GetComponent<UIPropRenameComponent>(node.path);
                var componentPropType = (propRenameComponent == null ? $"{node.name}Component" : propRenameComponent.propName).GetConfigTypeByAssembley();
        ...
        private static void TraverseUI(Transform parent, string path) {
            ...
            if (parent.childCount != 0 && parent.GetComponent<IUIIgnore>() == null) {
                ...
                var propRenameComponent = parent.GetComponent<UIPropRenameComponent>();
                var propType = (propRenameComponent == null ? $"{nodeName}Component" : propRenameComponent.propName).GetConfigTypeByAssembley();
                if (propType == null || nodes.Count == 0) {
