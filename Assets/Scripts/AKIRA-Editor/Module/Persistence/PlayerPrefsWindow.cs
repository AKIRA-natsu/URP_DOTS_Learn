...
public class PlayerPrefsWindow : EditorWindow {
    ...
    private static void ShowWindow() {
        GetWindowWithRect<PlayerPrefsWindow>(new Rect(Vector2.zero, new (780, 400)), false, "PlayerPrefsWindow");
    }
    ...
    public void CreateGUI() {
        ...
        viewContainer.Add(new VisualElement() { name = "Line", style = { backgroundColor = Color.black, height = 1f, }} );
...
internal class PrefDataView : MultiColumnListView {
    // 当前读取的路径
    private string path;
    ...
    public PrefDataView() : base() {
        ...
        columns.stretchMode = Columns.StretchMode.GrowAndFill;
    ...
    public void OnShowDataView(string path) {
        ...
        var width = EditorWindow.GetWindow<PlayerPrefsWindow>().position.width;

        columns.Add(new() { name = "key", title = "Key", width = width / 8f, makeCell = () => new Label(), bindCell = (e, i) => {
            (e as Label).text = lines[i].Split("||")[0];
        }});

        columns.Add(new() { name = "type", title = "Type", width = width / 8f, makeCell = () => new Label(), bindCell = (e, i) => {
            (e as Label).text = lines[i].Split("||")[2];
        }});

        columns.Add(new() { name = "value", title = "Value", width = width / 2f, makeCell = () => new(), bindCell = (e, i) => {
            e.Add(new Label(lines[i].Split("||")[1]));
        }});

        columns.Add(new() { name = "Operation", width = width / 4f, makeHeader = () => {
            var header = new VisualElement() { style = {
                alignItems = Align.Center, 
                justifyContent = Justify.Center,
                flexDirection = FlexDirection.Row,
            }};
            header.Add(new Button(() => System.Diagnostics.Process.Start(path)) { text = "Edit" } );
            header.Add(new Button(ClearView) { text = "Close" } );
            return header;
        }, makeCell = () => 
            ...
            e.Add(new Button(() => OnChangeRegisterValue(i)) { text = "Change" } );
            e.Add(new Button(() => OnDeleteRegisterValue(i)) { text = "Delete" } );
        }});
    }
    ...
    private void OnChangeRegisterValue(int index) {
        var valueElement = GetRootElementForIndex(index).ElementAt(2).ElementAt(0);
        var label = valueElement.Q<Label>();
        
        var btnElement = GetRootElementForIndex(index).ElementAt(3).ElementAt(0);

        if (label.visible) {
            ActiveElement(label, false);
            valueElement.Add(new TextField() { value = label.text });

            btnElement.Children().Foreach(child => ActiveElement(child, false));
            btnElement.Add(new Button(() => ApplyChanged(index, true)) { text = "ok" });
            btnElement.Add(new Button(() => ApplyChanged(index, false)) { text = "cancel" });
        } else {
            ActiveElement(label, true);
            btnElement.RemoveAt(btnElement.childCount - 1);
            btnElement.RemoveAt(btnElement.childCount - 1);
            btnElement.Children().Foreach(child => ActiveElement(child, true));
        }
    }
    ...
    private void ApplyChanged(int index, bool apply) {
        var valueElement = GetRootElementForIndex(index).ElementAt(2).ElementAt(0);
        var label = valueElement.Q<Label>();
        var field = valueElement.Q<TextField>();
        ...
        if (apply) {
            ...
            // change file value
            var key = $"{GetRootElementForIndex(index).ElementAt(0).ElementAt(0).Q<Label>().text}||";
        ...
        valueElement.Remove(field);
        OnChangeRegisterValue(index);
    }

    private void OnDeleteRegisterValue(int index) {
        var key = $"{GetRootElementForIndex(index).ElementAt(0).ElementAt(0).Q<Label>().text}||";
        ...
        // 通过该行父物体删除
        GetRootElementForIndex(index).parent.RemoveAt(index);
    }
